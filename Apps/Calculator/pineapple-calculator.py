#!/usr/bin/env python3
# =============================================================================
#  Pineapple Calculator — app GTK4 (a lógica vive em pineapple_calculator.py)
# =============================================================================
import gi
...
APP_ID = "org.pineappleos.calculator"


class PineappleCalculator(Gtk.Application):
    def __init__(self):
...
class CalculatorWindow(Gtk.ApplicationWindow):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.set_title("Pineapple Calculator")
        self.set_default_size(320, 460)
        self.add_css_class("pineapple-calc-window")

        self.display = Gtk.Entry()
        self.display.set_hexpand(True)
        self.display.set_alignment(1.0)
        self.display.set_css_classes(["pineapple-calc-display"])
    ...
    def _css_for(label):
        if label in ("=",):
            return ["pineapple-calc-accent"]
        if label in ("C", "⌫", "±", "%"):
            return ["pineapple-calc-fn"]
        if label in ("÷", "×", "−", "+"):
            return ["pineapple-calc-op"]
        return ["pineapple-calc-num"]
