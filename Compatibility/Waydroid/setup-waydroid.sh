#!/bin/bash
# =============================================================================
#  setup-waydroid.sh — instala o Waydroid (Android) no Mak OS
# =============================================================================
set -euo pipefail
...
cat > "$HOME/.local/bin/mak-waydroid" <<'EOF'
#!/bin/bash
# Atalho do Waydroid no Mak OS
case "${1:-start}" in
  ...
  *)      echo "uso: mak-waydroid [start|stop|apps|shell|install <apk>]";;
esac
EOF
chmod +x "$HOME/.local/bin/mak-waydroid"

echo "==> Waydroid pronto. Use:  mak-waydroid install app.apk"
