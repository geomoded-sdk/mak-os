#!/bin/bash
# =============================================================================
#  mak-appimage — gerenciador de AppImages do Mak OS
#
#  Registra AppImages no sistema (executável + entrada no Launcher).
#
#  Uso:
#    mak-appimage <arquivo.AppImage>       registrar um app
#    mak-appimage --list                   listar apps registrados
#    mak-appimage --remove <nome>          remover um app
# =============================================================================
set -euo pipefail
...
case "${1:-}" in
  ...
  --remove)
    ...
    ;;
  *)
    [ -n "${1:-}" ] || { echo "uso: mak-appimage <arquivo.AppImage>" >&2; exit 1; }
    src="$(realpath "$1")"
...
Type=Application
Name=$base
Comment=AppImage gerenciado pelo Mak OS
